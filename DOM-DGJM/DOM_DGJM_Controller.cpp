//DOM_DGM_Controller Etapa 6
//25/08/2024 - Grupo: DGJM
//Deborah Carvalho Saad 
//Gustavo Negrao de Souza Berengani Ramos
//Joao Pedro Tavares de Siqueira
		...
		do
		{
			...
			switch(opMenuJogador)
			{
				...
				case 3: //Passar					
					...
					retornofim = fim();
					break;
				case 4: //Saida para o menu principal
					system("cls");
					jogo();												
					break;
				default:
...
void jogarPeca() //Joga a peca na mesa
 {
	int escolha, retorno, pos, count, i, lado;
	contarPecasMesa();		
	escolha = menuJogada();
	...
	//Verificar se as pecas nas duas extremidades da mesa sao iguais
	if((peca[escolha].lado1 == mesa[QtdPecasMesa - 1].ladoD) || (peca[escolha].lado2 == mesa[QtdPecasMesa - 1].ladoD) && (peca[escolha].lado1 == mesa[0].ladoE) || (peca[escolha].lado2 == mesa[0].ladoE))
	{
		lado = menuLadoMesa();
		if(lado == 1) //Jogar no lado esquerdo
		{
